...
const MONGO_URI =
  'mongodb+srv://hamzasajidjadoon1997:uRPyCEjlLFMYaowN@td-cluster.joo5n.mongodb.net/?retryWrites=true&w=majority&appName=td-cluster';
const JWT_SECRET = '0106203abcdefgsvdsvvsrsya34s6dd7';

// Initialize Express app
const app = express();
const port = 3001; // Use the port from .env file
// Allow CORS for React Native Web on port 3000
app.use(cors({
  origin: 'http://localhost:3000',  // React Native Web URL
}));
...
app.post('/addTask', verifyToken, async (req, res) => {
  try {
    ...
    if (!taskTitle || !taskDetail) {
      return res
        .status(400)
        .json({message: 'Please provide task title and detail'});
    }
